...
class InputValidator:
    ...
    def __init__(self, input_data: Dict, validate_extentions : List[str], num_of_files: int =0) -> None:
        ...
        self.files=[]
        self.num_of_files=len(self.validate_extentions) if num_of_files==0 else num_of_files
    ...
    def _extract_uuid_and_check_validity(self) -> None:
        ...
        if not re.fullmatch(uuid_pattern, results_uuid_dir):
            raise ValueError(f"Invalid or missing UUID directory in results_path: {self.results_path}")

        # Check if the UUIDs match
        if context_uuid_dir != results_uuid_dir:
            raise ValueError(f"UUID mismatch between context_path and results_path: {context_uuid_dir} != {results_uuid_dir}")
    ...
    def _validate_paths(self):
        ...
        if len(context_path_parts) < 1 or not self._is_uuid_valid(context_path_parts[-1]):
            raise ValueError(f"Invalid context_path structure: {self.context_path}. "
        ...
        if len(results_path_parts) < 2 or results_path_parts[-1] != "out" or not self._is_uuid_valid(results_path_parts[-2]):
            raise ValueError(f"Invalid results_path structure: {self.results_path}. "
    ...
    def _validate_files(self):
        ...
        Raises:
            ValueError: If the context_path directory is empty, if the file extensions are invalid, or if there are not exactly one file per required extension.
        """
    ...
    def _validate_file_extensions(self):
        ...
        for file in self.files():
            ...
            if file_extension not in self.validate_extentions:
